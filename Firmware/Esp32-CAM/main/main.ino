/**************************
 * Copyright © 2025 LAB 14.0 - UEA Universidade do Estado do Estado do Amazonas
 * All rights reserved
 **************************/
...
const int kRxPin = 12;
const int kTxPin = 13;
const int kBaudRate = 115200; // Deve ser 115200 para sincronizar com a placa HTNB32L
...
void setup()
{
  ...
  // UART0: Debug no PC (conexão direta)
...
void loop()
{
  Serial.println("\nAguardando comando 'OK' na porta serial para capturar imagem...");

  // Aguarda até receber dados na serial
  ...
  {
    Serial.println("Comando 'OK' recebido! Capturando imagem...");
    delay(1000); // Pequeno atraso antes da captura
    ...
    for (int i = 0; i < numPacotes; i++)
    {
      ...
      for (int j = 0; j < size; j++)
      {
        ...
        {
          SerialApp.print("0");
          Serial.print("0"); // Envia para o cabo do PC para visualização
        }
        SerialApp.print(pixel, HEX);
        Serial.print(pixel, HEX); // Envia para o cabo do PC
      }
      SerialApp.println();
      Serial.println();

      // Delay otimizado de 300ms para estabilidade do buffer e rede celular
      delay(300);
    }
  ...
  {
    Serial.printf("Comando desconhecido recebido: '%s'\n", cmd.c_str());
  }

  // Limpa qualquer lixo que tenha ficado na serial (ex: enters sobrando)
  while (Serial.available() > 0)
  {
    Serial.read();
  }
